package com.srivastavavivekggn.scala.util.test.context
...
protected object AlwaysDirtiesContextTestExecutionListener extends DirtiesContextTestExecutionListener {
  ...
  override def afterTestClass(testContext: TestContext): Unit = {
    ...
    val hierarchyMode: DirtiesContext.HierarchyMode = Option(annAttrs).map(x => annAttrs.getEnum[DirtiesContext.HierarchyMode]("hierarchyMode")).orNull
